...
- `-s1, --scaffolds-1 TEXT`: Path to the list of scaffolds for the first 
mpileup file [required]

- `-s2, --scaffolds-2 TEXT`: Path to the list of scaffolds for the second 
mpileup file [required]
...
- `-c, --min-cov INTEGER`: Minimum coverage to consider a position (default: 
5)

- `-l, --min-gene-compare-len INTEGER`: Minimum gene length to consider for 
comparison (default: 100)

- `-m, --memory-mode [heavy|light]`: Memory mode for processing (default: 
heavy)

- `-b, --chrom-batch-size INTEGER`: Batch size for processing chromosomes 
(default: 10000)

- `-o, --output-file TEXT`: Path to save the parquet file [required]

- `-g, --genome TEXT`: If provided, do comparison only for the specified 
genome (default: all)

- `-e, --engine [streaming|gpu|auto]`: Engine to use for processing (default: 
streaming)
...
- `-e, --execution-mode TEXT`: Execution mode: 'local' or 'slurm' (default:  
local)

- `-s, --slurm-config TEXT`: Path to the SLURM configuration file in JSON  
format (required if execution mode is 'slurm')
...
- `-a, --polars-engine [streaming|gpu|auto]`: Polars engine to use (default: 
streaming)

- `-b, --chrom-batch-size INTEGER`: Batch size for processing chromosomes  
(default: 10000)

- `-h, --memory-mode [heavy|light]`: Memory mode for processing (default:  
heavy)
...
Generate  a table of pairs that are needed to complete the genome comparison database for a given profile database:

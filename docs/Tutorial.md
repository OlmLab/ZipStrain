...
!!! tip "Optional dN/dS preparation"
    Add `--gene-fasta genes.fna --prepare-dnds` to write a sparse `<sample>_codon_profile.parquet`. The same run adds reference-relative `ref_pN`/`ref_pS`/`ref_pN_pS` (observed proportions) and `ref_dN`/`ref_dS`/`ref_dN_dS` (Jukes-Cantor corrected) fields to `<sample>_gene_stats.parquet`. This path requires the reference FASTA and gene annotations, uses DuckDB with a `1GB` memory limit by default, and is skipped entirely unless requested.

...
│   ├── N5_271_010G1_codon_profile.parquet # optional codon alleles for dN/dS and pN/pS
...
!!! tip "Optional dN/dS preparation"
    Add `--gene-fasta genes.fna --prepare-dnds` to write `<sample>_codon_profile.parquet`. The same run adds reference-relative dN/dS from fixed SNSs and pN/pS from polymorphic SNVs to `<sample>_gene_stats.parquet`. Use `--allele-integration consensus` (default) to select the most-supported differing allele, averaging only exact top ties, or `weighted` to weight all differing alleles by frequency. This path requires the reference FASTA and gene annotations, uses DuckDB with a `1GB` memory limit by default, and is skipped entirely unless requested.
